...
vec3 BlinnPhong()
{
    ...
    vec3 LightDir = normalize(u_LightPos_vs - vPosition_vs);

    vec3 halfwayDir = (viewDir + LightDir) * 0.5;

    float d = distance(u_LightPos_vs, vPosition_vs);
    vec3 LightIntensity = u_LightIntensity / (d * d);
